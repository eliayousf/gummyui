...
| Convex                  | Production has the current 25-table schema/functions and now contains the controlled account/privacy journey rather than an empty database. Backup `20260729T123911183Z-2b453beb402d4f6d818aafde6ecf6f7d` encrypted, uploaded and independently verified all 24 durable tables and 26 records; a new empty isolated target restored and re-exported those same counts with `rateLimitWindows` still empty. After the membership retry, backup `20260729T125815872Z-36a3348ed93148cfad2fa6e193d8023a` independently verified 24 tables/28 records. Production was export-only. A fresh dedicated `gummyui-sandbox` project/deployment was created on 30 July, its empty 24-table target was seeded only with the synthetic restore-query identity/release fixture, and the paid-access, grant replay/expiry and refund-revocation proof passed without external integrations. The isolated two-offer and test-clock Stripe journeys also reconciled their exact purchase, licence, entitlement, seat, renewal, failed-invoice and access state. Founder-authenticated inspection then permanently deleted the completed fresh restore deployment and confirmed the older synthetic target already absent. After the final-deletion request, the current backup `20260730T231026109Z-9abf91335ff3450986d5cbbbfb555605` passed production and independent authenticated read-back for all 24 durable tables/53 records. | Preserve the isolated two-offer and test-clock evidence and complete the remaining production customer journeys. |
| WorkOS                  | The replacement production environment is fully configured and the real hosted sign-up/callback now passes. Dashboard inspection on 30 July reconfirmed the production app homepage as `https://gummyui.dev` and its redirect URI as `https://gummyui.dev/auth/callback`. The Convex CLI message that set `http://localhost:3000` was traced to the separate `gummyui-sandbox` development deployment and its WorkOS staging key; it did not target production. One active production profile, two workspaces and the owner/admin memberships are projected in Convex. Team creation/switching, export/download, deletion request/cancellation, session refresh and authenticated unpaid download denial passed. Signed user, organization and membership events were delivered. The live membership payload exposed an unused organization-name requirement; commit `2fb2b6b` fixes it with a real-payload regression test. WorkOS's 13:56 retry is Delivered and the matching Vercel request returned 200. Direct inspection on 30 July reconfirmed that the older orphaned environment and its platform-managed, non-expiring Convex credential still exist; no support confirmation has arrived. The controlled support invitation email was Delivered, the recipient accepted it, WorkOS showed two active production users, the scheduled signed `invitation.accepted` retry was Delivered, and Gummy UI/Convex reconciled the Accepted invitation plus two active admin/member memberships. The earlier deletion cancellation proves recovery; with the second operator retained, a fresh verified deletion request started the real seven-day cancellation window ending 6 August. A dashboard support follow-up was accepted without exposing secrets and protected the current replacement environment. The production privacy job returned HTTP 200 during the cancellation window and preserved the active request and cancellation control. | Let the real cancellation window expire, verify final provider erasure, and obtain support confirmation that the orphaned key/environment was revoked. |
| Resend                  | `send.kreydlabs.com` is verified. Production sending and the signed webhook are enabled. In addition to the deleted one-use controlled sender authorization, the real export and deletion/cancellation journey produced two application outbox messages; Resend and Convex delivery audits record both as delivered. A fresh 30 July inspection showed all three controlled messages Delivered and successful `email.delivered` webhook signals. The final-deletion request generated a fresh application status message, and Resend recorded its expected sender, subject, topic tag, account link and Delivered event at 00:05 BST on 31 July. | Complete purchase/licence/refund email journeys after Stripe and a protected release are ready. |
...
| Backblaze B2            | Two private encrypted EU buckets exist for protected releases and operational backups. The restored 26-record production backup remains the recovery proof. After the final-deletion request, current backup `20260730T231026109Z-9abf91335ff3450986d5cbbbfb555605` uploaded encrypted/authenticated objects and passed production plus independent authenticated read-back for all 24 durable tables/53 records. Founder dashboard reauthentication on 30 July exposed the exact three-key inventory; the pre-rotation backup key was reconciled against the current runtime and recovery configurations and revoked, while the release-read and dated current-backup keys remain. The surviving backup credential reauthenticated through isolated CLI profiles without retaining any key ID, object name or reusable credential in evidence. | Upload and verify immutable paid releases, prove paid delivery, and move the recovery bundle into founder-approved custody. |
...
1. Preserve the passing Lighthouse production audits (mobile performance 98;
   ...
   and zero failures.
2. Complete account recovery and final deletion while preserving the
   already-set `CONVEX_SERVER_SECRET`; preserve the now-passing second-identity
   invitation evidence.
