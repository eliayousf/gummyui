...
journey remain gated. Credential-rotation deployment
`dpl_Cc41SdA8BSeg1XGMDmdPQv2Jhe5c` is Ready from clean public head
`c9feeba856bc9380b401c6be3df5b11987501a0a`, both custom domains are Valid,
the full public-origin probe passes, and checkout and webhook flags remain fail
closed. GitHub Quality run `30407698983` passes the exact head.

## Production provider state — 29 July 2026
...
| Vercel                  | Pro is active, with spend management set to $1, notifications enabled and Pause Projects on. Every planned Production environment value is installed. The Stripe resource is globally restricted to Production. Ready deployment `dpl_Cc41SdA8BSeg1XGMDmdPQv2Jhe5c` was built from a clean checkout at exact public head `c9feeba856bc9380b401c6be3df5b11987501a0a` on Node 22 after the Convex and WorkOS rotations. Vercel marks the apex and `www` domains Valid, public DNS returns the current records, and the HTTPS health probe passes at `gummyui.dev` with commerce disabled. Controlled rollback and re-promotion were proved for the preceding audited release. | Preserve fail-closed flags, replace or explicitly accept the full-scope managed key only after security review, then verify every provider and customer journey at the real origin. |
| Convex                  | The EU development deployment exists; production has a rotated `CONVEX_SERVER_SECRET` and the replacement WorkOS deploy-time credentials. The current 25-table schema, indexes and functions, including protected release publication/withdrawal and `downloadGrants.by_release`, are deployed; post-deploy inspection confirmed all 25 tables are present and empty. An isolated development restore target has the same schema/functions and only drill-specific secrets. The latest backup restored and reconciled all 24 durable tables there with zero records; a subsequent synthetic drill proved representative account, team, licence, entitlement, one-use/expired download, refund-revocation and audit queries without invoking outbound providers. | Verify WorkOS identity at the production origin, complete the data-lifecycle journeys, then remove the synthetic drill target after founder dashboard authentication. |
| WorkOS                  | Staging journeys passed. Replacement production environment `environment_01KYNGX9WSHKMGFT7BYTW41PBE` has the exact callback/homepage, apex and `www` CORS origins, Gummy UI display branding, a Gummy UI sign-in endpoint, default sign-out redirect and an enabled webhook listening for the 13 implemented identity/team/invitation events. Email + Password uses the strong policy; Magic Auth and the four lifecycle email classes are enabled. Sessions are capped at 30 days with 5-minute access tokens and a 2-day inactivity timeout. Rotated credentials and the webhook secret are installed in Vercel and Convex production. WorkOS team support is `support@kreydlabs.com`. The real-origin sign-in route uses the replacement client and reaches hosted sign-up. An unused manually created key was expired immediately; WorkOS support is handling revocation/deletion of the unreachable orphaned platform-managed key/environment. | Complete the human CAPTCHA, callback and JWT integration, obtain WorkOS confirmation of the orphaned-key revocation, then repeat account creation, recovery, team, invitation, export and deletion journeys at the production origin. |
