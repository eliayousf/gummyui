...
export class FixedWindowRateLimiter {
  ...
  consume(key: string, now: number, cost = 1): RateLimitDecision {
    if (
      !key
      || key.length > 512
    ...
    if (window && now < window.startsAt) {
      return {
        allowed: false,
        remaining: 0,
        retryAfterMs: this.windowMs,
        resetAt: window.startsAt + this.windowMs,
      };
    }
    if (
      !window
      || now >= window.startsAt + this.windowMs
    ) {
      window = { startsAt: now, count: 0 };
      this.windows.set(key, window);
    }
    const resetAt = window.startsAt + this.windowMs;
    if (window.count + cost > this.capacity) {
      return {
        allowed: false,
        remaining: Math.max(0, this.capacity - window.count),
    ...
    return {
      allowed: true,
      remaining: this.capacity - window.count,
      retryAfterMs: 0,
      resetAt,
    };
  }
